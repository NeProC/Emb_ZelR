...
//    bubble_sort(arr, len_arr);
    ...
    int32_t len_neg_arr = counting_negative(arr, len_arr);
    int32_t len_pos_arr = counting_positiv(arr, len_arr);
    
    int32_t arr_pos[len_pos_arr];
    int32_t arr_neg[len_neg_arr];

    transfer_pos(arr, arr_pos, len_arr);
    transfer_neg(arr, arr_neg, len_arr);
    
    print_arr(arr_pos, len_pos_arr);
    print_arr(arr_neg, len_neg_arr);
    return 0;
}
